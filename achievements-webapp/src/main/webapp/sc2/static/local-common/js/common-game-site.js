		...
		for (var i = 0, type; type = config.resultTypes[i]; i++) {
			...
			if (groups[type]) {
				...
				for (var x = 0, item; item = groups[type][x]; x++) {
					...
					title = '<span class="title">' + this.highlight($('<div/>').text(item.title).html()) + '</span>';
...
var ReportPost = {
	...
	bindEvents: function() {
		...
		$("a[data-vote-type='down']", ReportPost.contextWrapper).on("click", function() {
			ReportPost.ratePost(this)
		});

		//bind all report nodes within the wrapper
		$("a[data-vote-type='report']", ReportPost.contextWrapper).on("click", function() {
		...
		var rateContext = (ReportPost.postType === "forums") ? ".topic-post" : ".comment-tile";

		/**
		 * IE6 and IE7 will have their post options always showing
		 */
		if (!Core.isIE(6) && !Core.isIE(7)) {
			$(ReportPost.contextWrapper)
				.on("mouseover", rateContext, function(event) {
					if (Core.loggedIn) {
						$(".rate-up, .downvote-wrapper", this)
							.not(".keep-shown")
	...
	openReportForm: function(node) {
		...
		ReportPost.formWrapper
			.insertAfter($('#post-' + postId))
			.show();
		$('#report-success').hide();
		$('#report-table').show();
